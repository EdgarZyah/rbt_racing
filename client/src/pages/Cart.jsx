...
export default function Cart() {
  ...
  // Mengambil alamat utama, jika tidak ada ambil array ke-0 (object tunggal)
  const mainAddress = addresses.find((addr) => addr.isMain) || addresses;
  ...
  const handleCheckoutClick = () => {
    ...
    if (user && !user.isVerified) {
      setModalConfig({
        ...
        message:
          "Your email is not verified. Please verify your account in the dashboard to proceed with checkout.",
    ...
    if (!mainAddress) {
      setModalConfig({
        ...
        message:
          "No shipping address detected. Would you like to set up your delivery address now?",
        ...
        <div className="mb-8 bg-red-50 border border-red-200 p-4 flex items-center justify-between">
          <div className="flex items-center gap-3 text-red-600">
            ...
            <p className="text-xs font-bold uppercase tracking-widest">
              Account unverified. Checkout disabled.
            </p>
          </div>
          <Link
            to="/customer"
            className="text-[10px] font-black uppercase underline text-red-600 hover:text-red-800"
          >
            Verify Now
          </Link>
        </div>
      ...
      {cartItems.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          <div className="lg:col-span-2 space-y-8">
            {cartItems.map((item) => {
              // Normalisasi path gambar agar tidak rusak saat digabung dengan base URL
              let imageUrl = null;
              if (item.image) {
                // Mengubah backslash (\) menjadi forward slash (/) jika ada (Khas Windows)
                ...
                if (
                  ...
                  // Memastikan ada '/' di antara APP_BASE_URL dan path gambar
                  const separator = normalizedPath.startsWith("/") ? "" : "/";
                  imageUrl = `${APP_BASE_URL}${separator}${normalizedPath}`;
                }
              }
              ...
              return (
                <div
                  key={item.cartId}
                  className="flex flex-col sm:flex-row gap-6 pb-8 border-b border-zinc-50"
                >
                  <div className="w-full sm:max-w-xs aspect-square bg-zinc-50 border border-zinc-100 flex-shrink-0 overflow-hidden flex items-center justify-center">
                    {imageUrl ? (
                      <img
                        src={imageUrl}
                        alt={item.name}
                  ...
                  <div className="flex-grow text-left">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-400 uppercase italic font-bold mb-2">
                          {item.category || "Gear"}
                        </p>

                        {/* Render Variants Info if exists */}
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                              {Object.entries(item.selectedVariants).map(
                                ([key, value]) => (
                                  <span
                                    key={key}
                                    className="bg-zinc-100 text-[9px] font-bold uppercase px-2 py-1 text-zinc-500"
                                  >
                                    {value}
                                  </span>
                                ),
                    ...
                    <div className="mt-8 flex justify-between items-center">
                      <div className="flex items-center border border-zinc-200">
                        <button
                          ...
                          className="p-3 hover:bg-zinc-50 transition border-r border-zinc-200 text-zinc-500 disabled:opacity-30"
                        >
                          <Minus size={12} />
                        </button>
                        ...
                        <button
                          ...
                          className="p-3 hover:bg-zinc-50 transition border-l border-zinc-200 text-zinc-500 disabled:opacity-30"
                        >
                          <Plus size={12} />
                        </button>
                      </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-dashed border-zinc-200">
                      ...
                      <input
                        ...
                        placeholder="instructions for seller..."
                        onBlur={(e) => {
                          // Menggunakan context agar UI React dan Database tersinkron dengan baik
                          updateItemNote(item.cartId, e.target.value);
                        }}
                        className="w-full bg-zinc-50 border border-zinc-200 p-3 text-xs focus:bg-white focus:outline-none focus:border-black transition"
                      />
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-zinc-50 p-8 h-fit border border-zinc-100 sticky top-24 text-left">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-8 border-b border-zinc-200 pb-4">
              Order Summary
            </h2>
              ...
              {loadingAddress ? (
                ...
                <div className="bg-white border border-zinc-200 p-4 relative group">
                  ...
                  <p className="text-xs font-black uppercase mb-1">
                    {mainAddress.receiverName}
                  </p>
                  <p className="text-[10px] text-zinc-500 leading-relaxed line-clamp-2">
                    {mainAddress.fullAddress}, {mainAddress.district},{" "}
                    {mainAddress.city}
                  </p>
                  <Link
                    to="/customer/address"
                    className="absolute top-4 right-4 text-[9px] font-bold uppercase text-black hover:underline bg-zinc-100 px-2 py-1"
                  >
                    Change
                  </Link>
                </div>
                ...
                <div className="bg-red-50 border border-red-100 p-4 text-center">
                  ...
                  />
                  <p className="text-xs font-bold text-red-500 uppercase mb-3">
                    Shipping Address Missing
                  </p>
                  <Link
                    to="/customer/address"
                    className="block w-full bg-red-500 text-white py-2 text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition"
                  >
                    Set Address Now
                  </Link>
                </div>
        ...
        <div className="py-32 text-center bg-zinc-50 border border-dashed border-zinc-200">
          <ShoppingBag
            size={32}
            className="mx-auto text-zinc-300 mb-6"
            strokeWidth={1.5}
          />
